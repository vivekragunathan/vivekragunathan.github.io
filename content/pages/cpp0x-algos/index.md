...
namespace stdext
{
    ...
    template<typename TIterator, typename TPredicate>
    TIterator TrueForAny(
			TIterator begin,
			TIterator end,
			TPredicate predicateFunc
		)
    {
        while (begin != end)
        {
    ...
    TContainer<TOutType, std::allocator<TOutType>>
            ConvertAll(
                const TContainer<TInType,
                std::allocator<TInType>>& src,
                TConverter convFunc
            )
    {
    ...
    template <typename T> bool FromString(
        const String& text,
        T& tObj,
    ...
    {
    public:
        virtual TOut operator()(const TIn& tin)
        {
